module ExceptionHandler
  ...
  BOTS = %w[Baidu Gigabot Googlebot libwww-per lwp-trivial msnbot SiteUptime \
            Slurp Wordpress ZIBB ZyBorg Yandex Jyxobot Huaweisymantecspider \
            ApptusBot].freeze

  # => Attributes
  # => Determine schema etc
  ATTRS = %i[class_name status message trace target referrer params \
             user_agent].freeze

  # => Exceptions to be rescued by ExceptionHandler
  EXCEPTIONS_TO_BE_RESCUED = [ActionController::RoutingError, \
                              AbstractController::ActionNotFound].tap do |list|
  ...
  self::Exception = \
    Class.new( \
      ...
      #                 @request.env, @exception).status_code
      ...
      if ExceptionHandler.config.try(:email).try(:is_a?,
                                                 String)
        after_initialize (proc { |e|
      ...
      # => Validations
      validates :klass, exclusion:    { in: EXCEPTIONS_TO_BE_RESCUED,
                                        message: '%<value>s' },
                        if: -> { referer.blank? } # => might need full Proc syntax
      validates :user_agent, format:  { without: Regexp.new(BOTS.join('|'),
                                                            Regexp::IGNORECASE) }
      ...
      # => Description
      def description
        I18n.with_options scope: [:exception_handler],
                          message: message,
                          status: status do |i18n|
          i18n.t response,
                 default: Rack::Utils::HTTP_STATUS_CODES[status] || status
        end
      end
